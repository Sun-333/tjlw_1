...
//                System.out.println("-------------------------------------------");
    ...
    public boolean deleteRows(String tableName, String ... rowKeys) {
        ...
        try {
            ...
            if(admin.tableExists(TableName.valueOf(tableName))){
                ...
                log.debug(MessageFormat.format("row({0}) is deleted!",deleteList));
            }
        }catch (IOException e) {
            log.error("删除多行失败");
            return false;
     ...
     * @throws IOException
     */
    public boolean deleteRows(String tableName,String endTime) throws IOException {
        Table table = connection.getTable(TableName.valueOf(tableName));//借助connection创建表连接————创建连接池
        Scan scan_find = new Scan();
        scan_find.withStopRow(Bytes.toBytes(endTime));
        ResultScanner rs = table.getScanner(scan_find);
    ...
    /*
 public int count(String tableName,Scan scan){
        try {
            AggregationClient
